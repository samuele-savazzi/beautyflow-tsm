import 'package:test/test.dart';
import 'package:beautyflow_api/beautyflow_api.dart';

// tests for ConsultationDetailResponse
void main() {
  final instance = ConsultationDetailResponseBuilder();
  // TODO add properties to the builder and call build()

  group(ConsultationDetailResponse, () {
    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // int areaId
    test('to test the property `areaId`', () async {
      // TODO
    });

    // String areaName
    test('to test the property `areaName`', () async {
      // TODO
    });

    // int operatorId
    test('to test the property `operatorId`', () async {
      // TODO
    });

    // String operatorName
    test('to test the property `operatorName`', () async {
      // TODO
    });

    // int customerId
    test('to test the property `customerId`', () async {
      // TODO
    });

    // String customerName
    test('to test the property `customerName`', () async {
      // TODO
    });

    // String customerEmail
    test('to test the property `customerEmail`', () async {
      // TODO
    });

    // String customerPhone
    test('to test the property `customerPhone`', () async {
      // TODO
    });

    // int serviceId
    test('to test the property `serviceId`', () async {
      // TODO
    });

    // String serviceName
    test('to test the property `serviceName`', () async {
      // TODO
    });

    // JsonObject serviceStepsSnapshot
    test('to test the property `serviceStepsSnapshot`', () async {
      // TODO
    });

    // String serviceDescription
    test('to test the property `serviceDescription`', () async {
      // TODO
    });

    // JsonObject serviceQuestions
    test('to test the property `serviceQuestions`', () async {
      // TODO
    });

    // JsonObject answers
    test('to test the property `answers`', () async {
      // TODO
    });

    // int bookingId
    test('to test the property `bookingId`', () async {
      // TODO
    });

    // Date bookingDate
    test('to test the property `bookingDate`', () async {
      // TODO
    });

    // String bookingStartTime
    test('to test the property `bookingStartTime`', () async {
      // TODO
    });

    // String bookingEndTime
    test('to test the property `bookingEndTime`', () async {
      // TODO
    });

    // int bookingDuration
    test('to test the property `bookingDuration`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // String statusDisplay
    test('to test the property `statusDisplay`', () async {
      // TODO
    });

    // String notes
    test('to test the property `notes`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // DateTime completedAt
    test('to test the property `completedAt`', () async {
      // TODO
    });

    // bool canBeConverted
    test('to test the property `canBeConverted`', () async {
      // TODO
    });

    // bool canBeCancelled
    test('to test the property `canBeCancelled`', () async {
      // TODO
    });

  });
}
